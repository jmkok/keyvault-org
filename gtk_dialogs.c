...
gchar* gtk_dialog_password(GtkWindow* parent, const gchar* title) {
	...
	GtkWidget* dialog = gtk_dialog_new_with_buttons ("Password",
																	 ...
																	 NULL, NULL);
	GtkWidget* ok_button = gtk_dialog_add_button (GTK_DIALOG (dialog), "_OK", GTK_RESPONSE_OK);
  ...
  gtk_container_add(GTK_CONTAINER(content_area), vbox);
	...
	g_signal_connect_swapped (G_OBJECT (passphrase_entry), "activate", G_CALLBACK (gtk_button_clicked), ok_button);
...
void change_location_protocol(GtkWidget *widget, gpointer data) {
	...
	gchar* protocol = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(widget));
...
gboolean gtk_dialog_request_config (GtkWidget* parent, struct FILE_LOCATION* loc) {
	...
	ui->dialog = gtk_dialog_new_with_buttons ("File configuration",
																	 ...
																	 //~ "_Cancel", GTK_RESPONSE_CANCEL,
																	 NULL, NULL);
	GtkWidget* ok_button = gtk_dialog_add_button (GTK_DIALOG (ui->dialog), "_OK", GTK_RESPONSE_OK);
	gtk_dialog_add_button (GTK_DIALOG (ui->dialog), "_Cancel", GTK_RESPONSE_CANCEL);

	//~ gtk_window_set_icon_from_file(GTK_WINDOW(dialog), "/usr/share/pixmaps/apple-red.png", NULL);
	gtk_window_set_icon_name(GTK_WINDOW(ui->dialog), "dialog-password");
  ...
  gtk_box_pack_start(GTK_BOX(vbox), label , FALSE, TRUE, 1);
	...
	gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combo_box),"local");
	gtk_combo_box_text_append_text(GTK_COMBO_BOX_TEXT(combo_box),"ssh");
  ...
  g_signal_connect(G_OBJECT(combo_box), "changed", G_CALLBACK(change_location_protocol), ui);
	...
	//~ gtk_signal_connect_object (GTK_OBJECT (passphrase_entry), "focus_in_event", GTK_SIGNAL_FUNC (gtk_widget_grab_default), GTK_OBJECT (ok_button));
	g_signal_connect_swapped (G_OBJECT (combo_box), "activate", G_CALLBACK (gtk_button_clicked), ok_button);
	...
	if (gtk_dialog_run(GTK_DIALOG(ui->dialog)) == GTK_RESPONSE_OK) {
		...
		char* p = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(combo_box));
		if (p) {
...
void quick_message (GtkWidget *parent, gchar *message) {
	...
	gtk_container_add(GTK_CONTAINER(scroll),info_text);
